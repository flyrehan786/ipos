...
export class SaleOrderFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.orderForm = this.formBuilder.group({
      order_number: ['SO' + Date.now(), Validators.required],
      client_id: [null],
  ...
  loadClients(): void {
    this.clientService.getAll().subscribe({
      next: (data) => {
        this.clients = data.filter(c => c.status === 'active');
      }
    });
  }

  loadOrder(): void {
    this.saleOrderService.getById(this.orderId!).subscribe({
      next: (order) => {
  ...
  addItem(): void {
    ...
    const unitPrice = this.selectedProduct.sale_price;
    const total = (this.itemQuantity * unitPrice) - this.itemDiscount;
  ...
  onSubmit(): void {
    ...
    const request = this.isEditMode
      ? this.saleOrderService.update(this.orderId!, orderData)
      : this.saleOrderService.create(orderData);
